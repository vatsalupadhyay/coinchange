...
DPChange.TABLE_START_X = 600;
DPChange.TABLE_START_Y = 50;
DPChange.TABLE_DIFF_X = 70;

DPChange.CODE_START_X = 40;
DPChange.CODE_START_Y = 40;
DPChange.CODE_LINE_HEIGHT = 20;

DPChange.GREEDY_START_X = 200;
DPChange.GREEDY_START_Y = 250;
DPChange.RECURSIVE_START_X = 220;
DPChange.RECURSIVE_START_Y = 10;
DPChange.RECURSIVE_DELTA_Y = 20;
DPChange.RECURSIVE_DELTA_X = 8;
DPChange.CODE_HIGHLIGHT_COLOR = "#0099FF";
  ...
  for (i = 0; i < this.code.length; i++) {
    ...
    for (j = 0; j < this.code[i].length; j++) {
      ...
      this.cmd(
        ...
        this.codeID[i][j],
        this.code[i][j],
        DPChange.CODE_START_X,
        DPChange.CODE_START_Y + i * DPChange.CODE_LINE_HEIGHT,
        0
      );
      this.cmd(
        "SetForegroundColor",
        this.codeID[i][j],
        DPChange.CODE_STANDARD_COLOR
      );
      if (j > 0) {
...
DPChange.prototype.addControls = function () {
  ...
  this.fibField.onkeydown = this.returnSubmit(
    this.fibField,
  ...
  this.coinTypeButtons = addRadioButtonGroupToAlgorithmBar(
    coinLabels,
    "CoinType"
  );

  for (i = 0; i < this.coinTypeButtons.length; i++) {
    this.coinTypeButtons[i].onclick = this.coinTypeChangedCallback.bind(
      this,
      i
    );
...
DPChange.prototype.greedyCallback = function (value) {
  if (this.fibField.value != "") {
    this.implementAction(
      this.implementGreedy.bind(this),
      parseInt(this.fibField.value)
    );
  } else {
...
DPChange.prototype.implementGreedy = function (value) {
  ...
  this.cmd(
    ...
    DPChange.GREEDY_START_X,
    DPChange.GREEDY_START_Y
  );
  ...
  this.cmd(
    ...
    DPChange.TABLE_ELEM_HEIGHT,
    DPChange.GREEDY_START_X,
    DPChange.GREEDY_START_Y + DPChange.TABLE_ELEM_HEIGHT
  );
  ...
  for (i = DPChange.COINS[this.coinIndex].length - 1; i >= 0; i--) {
    while (value >= DPChange.COINS[this.coinIndex][i]) {
      ...
      this.cmd(
        "CreateLabel",
        moveIndex,
        DPChange.COINS[this.coinIndex][i],
        DPChange.GREEDY_START_X,
        DPChange.GREEDY_START_Y
      );
  ...
  this.cmd(
    ...
    "changeGreedy(" +
      String(initialValue) +
      ", [" +
      String(DPChange.COINS[this.coinIndex]) +
      "])	= " +
      String(requiredCoins),
    DPChange.RECURSIVE_START_X,
    DPChange.RECURSIVE_START_Y,
    0
  );
...
DPChange.prototype.buildTable = function (maxVal) {
  ...
  var xPos;
  var yPos;
  var table_rows = Math.floor(
    (this.canvasHeight - DPChange.TABLE_ELEM_HEIGHT - DPChange.TABLE_START_Y) /
      DPChange.TABLE_ELEM_HEIGHT
  );
  ...
  this.cmd(
    ...
    "Coins to Use",
    DPChange.TABLE_START_X +
      table_cols * DPChange.TABLE_DIFF_X +
      1.5 * DPChange.TABLE_DIFF_X,
    DPChange.TABLE_START_Y - 30
  );

  for (i = 0; i <= maxVal; i++) {
    yPos =
      (i % table_rows) * DPChange.TABLE_ELEM_HEIGHT + DPChange.TABLE_START_Y;
    xPos =
      Math.floor(i / table_rows) * DPChange.TABLE_DIFF_X +
      DPChange.TABLE_START_X;
    ...
    for (j = 0; j < 2; j++) {
      ...
      this.cmd(
        ...
        indexID,
        i,
        xPos - DPChange.TABLE_ELEM_WIDTH,
        yPos
      );
      this.cmd("SetForegroundColor", indexID, DPChange.TABLE_INDEX_COLOR);

      xPos =
        xPos + table_cols * DPChange.TABLE_DIFF_X + 1.5 * DPChange.TABLE_DIFF_X;
    }
  }
};
...
DPChange.prototype.displayCoinsUsed = function () {
  ...
  while (currValue > 0) {
    ...
    this.cmd(
      "CreateLabel",
      moveID,
      this.tableVals[1][currValue],
      this.tableXPos[1][currValue],
      this.tableYPos[1][currValue]
    );
...
DPChange.prototype.recursiveCallback = function (event) {
  ...
  if (this.fibField.value != "") {
    var fibValue = Math.min(
      parseInt(this.fibField.value),
      DPChange.MAX_VALUE - 5
    );
...
DPChange.prototype.change = function (value, xPos, ID) {
  ...
  this.cmd(
    "CreateLabel",
    ID,
    "change(" + String(value) + ", [" + String(coins) + "])",
    xPos,
    this.currentY,
    0
  );
  ...
  if (value > 0) {
    ...
    for (i = 0; i < coins.length; i++) {
      ...
      if (value >= coins[i]) {
        ...
        var nextTry = this.change(
          value - coins[i],
          xPos + DPChange.RECURSIVE_DELTA_X,
          recID
        );
...
DPChange.prototype.tableChange = function (value) {
  ...
  this.cmd(
    "CreateLabel",
    finalID,
    this.tableVals[0][value],
    this.tableXPos[0][value] - 5,
    this.tableYPos[0][value] - 5,
    0
  );
  this.cmd(
    "Move",
  ...
  this.cmd(
    "SetText",
    finalID,
    "change(" + String(value) + ") = " + String(this.tableVals[0][value])
  );
...
DPChange.prototype.fibMem = function (value, xPos, ID) {
  ...
  if (this.tableVals[value] >= 0) {
    ...
    this.cmd(
      "CreateLabel",
      ID,
      this.tableVals[value],
      this.tableXPos[value] - 5,
      this.tableYPos[value] - 5,
      0
    );
  ...
  if (value > 1) {
    ...
    this.cmd(
      "SetForegroundColor",
      this.codeID[4][1],
      DPChange.CODE_RECURSIVE_1_COLOR
    );
    ...
    this.cmd(
      "SetForegroundColor",
      this.codeID[4][3],
      DPChange.CODE_RECURSIVE_2_COLOR
    );
    ...
    this.cmd(
      "Move",
      this.nextIndex,
      this.tableXPos[value],
      this.tableYPos[value],
      this.currentY
    );
    this.cmd("Step");
